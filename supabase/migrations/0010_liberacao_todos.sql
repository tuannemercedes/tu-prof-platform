-- Opção "Todos os alunos" em toda liberação (materiais, cronograma,
-- planner, clube), pra não precisar marcar turma por turma / aluno por
-- aluno quando é pra todo mundo ver. Também cobre automaticamente alunos
-- novos que entrarem depois, sem precisar liberar de novo.

alter table public.materiais add column visivel_todos boolean not null default false;
alter table public.cronogramas add column visivel_todos boolean not null default false;
alter table public.planners add column visivel_todos boolean not null default false;
alter table public.clube_config add column visivel_todos boolean not null default false;
...
create policy "clube_temas: aluno select" on public.clube_temas
  for select using (
    exists (select 1 from public.clube_config cc where cc.visivel_todos = true)
    or exists (select 1 from public.clube_alunos ca where ca.aluno_id = auth.uid())
    or exists (
      select 1 from public.clube_turmas ct
      join public.turma_membros tm on tm.turma_id = ct.turma_id
